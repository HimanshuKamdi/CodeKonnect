...
const Channels = (props) => {
    ...
    if (suggestInput.length > 0) {
      let searchResults = props.users.filter((user) => {
        return user.displayName
          .toLowerCase()
          .includes(suggestInput.toLowerCase());
      });
  ...
  useEffect(() => {
    ...
    channelsState.map((channel) => {
      if (
        channel.members &&
        channel.members.some((member) => member.uid === props.user.uid)
      ) {
  ...
  const handleSelectUser = (user) => {
    if (channelAddState.members.includes(user)) {
      setChannelAddState((currentState) => {
        const updatedMembers = currentState.members.filter(
          ...
          <Form onSubmit={onSubmit}>
            <Segment stacked>
                  ...
                  {suggestions.map((user) => (
                    <li
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        cursor: "pointer",
                        backgroundColor: channelAddState.members.includes(user)
                          ? "green"
                          : "transparent",
                        margin: "10px",
                      }}
              ...
              {props.user?.git ? (
                ...
                <p style={{ color: "red" }}>
                  Add your git details to create github repo
                </p>
              )}
              {createNewRepo ? (
                <Form.Input
